...
<<<<<<< HEAD
        ...
        this.delaySlider = new DelaySliderWidget(
                centerX - sliderWidth/2, centerY + 55,
                sliderWidth, 20,
                initialDelayValue
        ) {
            @Override
            ...
            @Override
            protected void onValueChanged(int newValue) {
                delayField.setText(String.valueOf(newValue));
            }
        };
        this.addDrawableChild(this.delaySlider);
        ...
        this.delayField.setText(String.valueOf(initialDelayValue));
        this.delayField.setChangedListener((text) -> {
            try {
                int newValue = Integer.parseInt(text);
                if (newValue >= 0 && newValue <= 200) {
                    delaySlider.setValue(newValue);
                }
            } catch (NumberFormatException ignored) {}
        });
...
<<<<<<< HEAD
    ...
    private static abstract class DelaySliderWidget extends BaseSliderWidget {
        private double sliderValue;
        private final double minValue = 0;
        private final double maxValue = 200;

        protected DelaySliderWidget(int x, int y, int width, int height, int initialValue) {
            super(x, y, width, height, Text.empty());
            setValue(initialValue);
        }
